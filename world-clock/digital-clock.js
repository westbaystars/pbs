...
class DigitalClock {
  constructor() {
    this._width = 250;   // 30 x 6 digits + 15 x 2 colons + 30 x 1 AM/PM indicator
    this._height = 50;
  }
...
function drawDigits(svg, digits) {
  ...
  svg.selectAll('g.digit')
    .data(data)
    .join(
      enter => digitalClock.enterDigits(enter),
      update => digitalClock.updateDigits(update),
      exit => exit.remove()
    )
