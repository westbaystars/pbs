(function(global, factory) {
	...
	bs.api.modal.click	= function() {
		...
		var target = bs.api.find.target(this, 'modal'),
			...
			if (target.classed("fade")  && bs.api.transitionEvent != null)
			  backdrop.transition().duration(750).ease(d3.easeLinear).style('opacity',0.0).remove();
			else
				backdrop.remove();
			...
			if (target.classed("fade")  && bs.api.transitionEvent != null) {
				target.transition().duration(750).ease(d3.easeLinear).style('opacity',0.0);
				hide2(target);
			} else {
				hide2(target);
			}
      ...
      target.style('display', 'block');
		...
		backdrop = d3.select('body').append('div').attr('class', 'modal-backdrop').classed('fade', target.classed('fade')).style('opacity',0);
		backdrop.node().offsetWidth; // force reflow animation
		trigger(target,'show.bs.modal');
		if (target.classed('fade')  && bs.api.transitionEvent != null) {
  		backdrop.transition().duration(750).ease(d3.easeLinear).style('opacity',0.5);
      show();
		} else {
			show();
	  }
